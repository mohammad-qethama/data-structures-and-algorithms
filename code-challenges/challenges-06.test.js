...
const checkValues = (obj, value) => {
  ...
  if (valuesOfObject.includes(value)){return true;}
  else{return false;}
};
...
const updateNumbers = (obj) => {
  ...
  for (let index = 0; index < names.length; index++) {
    let nameObj = names[index];
    let numberObj = numbers[index];
    newArr.push(`${nameObj}: ${numberObj}`);
...
const hasChildrenValues = (arr, character) => {
  let charIndex = arr.map(function(e) { return e.name; }).indexOf(character);
  if (charIndex < 0){return false;}
  if (arr[charIndex].children.length){return true ;}
  else{return false;}
  // Solution code here...
