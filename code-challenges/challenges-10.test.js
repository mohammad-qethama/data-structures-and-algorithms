...
const totalSum = (input) => {
  ...
  return newArray.reduce((acc,input,index)=>{
    acc = acc+input;
    return acc;
  },0) ;
  // Solution code here...
...
const divisibleByFiveTwoToThePower = (input) => {
  let newArray = input.map(arr =>{
    return arr.filter(nums=>{ if (typeof nums === 'number' && isFinite(nums)){return !(nums%5);} }).map(elme=>{ return Math.pow(2,elme); });
...
let findMaleAndFemale = (data) => {
  return data.filter(obj => {if(obj.gender=== 'male' || obj.gender ==='female'){return true;}}).map(obj=> {return obj.name;}).join(' and ');
  // Solution code here...
};
...
let findShortest = (data) => {
  ...
  return data.map(obj=> {return obj.name;}).sort((a,b)=>
  { if (a.length>b.length){return a.length-b.length;}
    if (a.length < b.length){return -1;}
    else {return -1;}} )[0];// Solution code here...
};
