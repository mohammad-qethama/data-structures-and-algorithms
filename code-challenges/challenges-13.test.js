...
const longestString = (arr) => {
  let comp = 's';
  return arr.reduce((longestIndex,string,index) => {
...
const findHappiness = (arr) => {
  return arr.filter(string => {return string.includes(':)');});
  // Solution code here...
};
...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    if(i%2){
      element = element + str.substring(i,i+1);
    }
  }
  return element;
  // Solution code here...
...
const allHappy = (arr) => {
  let myArr = arr;
  let happyArr = arr.filter(string =>{
    return string.includes(':)');
  });
  return (myArr.length === happyArr.length) ? true : false;
  // Solution code here...
};
