...
const screenForNames = (arr) => {
  let newArr= arr.filter(names =>{
    let regExp =/^(Mr\.|Mrs\.|Ms\.|Dr\.)[\s][A-Z]/g;
    if(regExp.test(names)){return true;}
    else{return 0;}
  });
  return newArr;
  // Solution code here...
...
let biggerThanLuke = (arr) => {
  let arr1 = arr.filter(char=>{
    if(parseInt(char.mass) >77){
      return 1;
    }else{return false;}
  });
  let strGreater = arr1.map(char =>{
    return char.name;
  }).join(' - ');
  return strGreater;
  // Solution code here...
...
const sortBy = (property, arr) => {
  ...
  return arr.sort((a,b)=>{
    // (a.property>b.property)? return -1: return 1;
    if(a[property]>b[property]){return 11;}
    else{return -1;}
  });
  // Solution code here...
};
...
const isSecure = (url) => {
  let regExp = /^h.*\//g;
  if (url.match(regExp)){
    if(url.match(regExp)[0]==='https://'){return true;}
    else{return false;}
  }else{
    return false;
  }
...
const detectTicTacToeWin = (board) => {
  if(helpCheck(board[0][0],board[1][1],board[2][2])==='win'){return true;}

  for (let i in board){
    // console.log('hi')
    if(helpCheck(board[i][0],board[i][1],board[i][2])==='win'){return true;}
    if(helpCheck(board[0][i] ,board[1][i] ,board[2][i] )==='win'){return true;}
