...
const allUpperCase = (arr) => {
  ...
  let localArr = [];
  arr.forEach(arrelement => {
    localArr.push(arrelement.toUpperCase());
...
const addNumbers = (num, arr, times, callback) => {
  let arrTimes = Array.apply(null, Array(times));
  arrTimes.forEach(()=> {
    callback(arr,num);
...
const createList = (availableItems) => {
  let groceryListArr = [];
  availableItems.forEach( (itemObject) => {

    if (itemObject.available){
      groceryListArr.push(itemObject.name);
    }
  } );
